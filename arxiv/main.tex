...
\begin{enumerate}
\item \textbf{Steering effectiveness is at ceiling for small models and declines at larger scales.} Across the Qwen family (which holds architecture constant while varying only scale), coherent refusal rates are: 100\% at 3B $\to$ 100\% at 7B $\to$ 90\% at 14B $\to$ 77\% at 32B (DIM, optimal layer, n=30).\footnote{The canonical 32B rate uses our 30-prompt evaluation set. An earlier 50-prompt scaling sweep yielded 60\%. We use 77\% (n=30) as the canonical value throughout and note both where relevant. See \S\ref{sec:inverse-scaling} for details.} The actual decline begins only at 14B; at smaller scales effectiveness is at ceiling. This pattern is consistent with refusal geometry becoming more complex at larger scales, but we note that n=30 yields wide confidence intervals: 77\% and 90\% are not statistically distinguishable at this sample size.
...
\textbf{Reconciling with Beaglehole et al.} Our finding appears to contradict Beaglehole et al. \citep{beaglehole2025universal}, who find that larger models are \emph{more} steerable. The reconciliation is methodological: their approach uses RFM (a nonlinear kernel method) with all-block steering and 768 training examples, while ours uses DIM (linear, mean-difference) with a single direction applied from one target layer onward and $\sim$10 contrastive pairs. The gap between these results quantifies the scaling wall separating simple linear from complex nonlinear methods. One interpretation: the ``refusal direction'' that DIM extracts is a real feature at small scales but an increasingly lossy summary of a higher-dimensional structure at large scales, and nonlinear methods succeed precisely because they can represent this complexity. We note, however, that we have not run RFM ourselves; this reconciliation is inferred from published results under different experimental conditions.
...
\subsection{Transfer Coverage Matrix}

Table~\ref{tab:coverage} shows which model pairs were tested for direct behavioral transfer and which were excluded due to hidden-dimension mismatch. Direct vector transfer (adding a source direction to the target's residual stream) requires matching hidden dimensionality; dimension-mismatched pairs would require explicit projection or mapping, introducing a confound we chose to avoid.

\begin{table}[t]
\centering
\caption{Transfer coverage matrix. \textbf{Tested}: direct behavioral transfer run with TE reported. \textbf{Dim-mismatch}: hidden dimensions differ; not tested for behavioral transfer (would require projection). \textbf{Cross-family}: tested where dimensions match. All behavioral transfer claims are restricted to dim-matched, directly-tested pairs only.}
\label{tab:coverage}
\small
\begin{tabular}{@{}lcccccc@{}}
\toprule
Source $\downarrow$ / Target $\rightarrow$ & Q-3B & Q-7B & Q-14B & Q-32B & G-9B \\
 & \footnotesize{(2048)} & \footnotesize{(3584)} & \footnotesize{(5120)} & \footnotesize{(5120)} & \footnotesize{(3584)} \\
\midrule
Qwen 3B \footnotesize{(2048)} & self & mismatch & mismatch & mismatch & mismatch \\
Qwen 7B \footnotesize{(3584)} & mismatch & self & mismatch & mismatch & \textbf{tested} \\
Qwen 14B \footnotesize{(5120)} & mismatch & mismatch & self & \textbf{tested} & mismatch \\
Qwen 32B \footnotesize{(5120)} & mismatch & mismatch & \textbf{tested} & self & mismatch \\
Gemma 9B \footnotesize{(3584)} & mismatch & \textbf{tested} & mismatch & mismatch & self \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Key constraint.} Within the Qwen family, only 14B and 32B share hidden dimensionality (5120), making them the only same-family pair eligible for direct transfer without projection. Similarly, Qwen 7B and Gemma 9B are the only cross-family pair sharing hidden dimensionality (3584). All behavioral transfer conclusions in this paper are anchored on these two dim-matched pairs. Dimension-mismatched pairs (e.g., Qwen 3B $\leftrightarrow$ 7B, Qwen 7B $\leftrightarrow$ 14B) were not used for behavioral transfer and no claims are made about them.
...
\textbf{Transfer coverage.} Cross-architecture transfer was tested for one same-family pair (Qwen 14B $\leftrightarrow$ 32B, where transfer succeeded) and one cross-family pair (Qwen 7B $\leftrightarrow$ Gemma 9B, where transfer failed; see \S\ref{sec:transfer}). All transfer claims are anchored on these two dim-matched pairs; no claims are made about dimension-mismatched pairs. Broader family coverage --- additional same-family scales, additional cross-family pairs (e.g., Llama $\leftrightarrow$ Qwen), and investigation of whether shared pretraining data improves cross-family transfer --- remains future work.
...
\subsection{Not covered in this study}
The following are explicitly out of scope and should not be inferred from our results:
\begin{itemize}
    \item Refusal \emph{suppression} on harmful prompts (we test only refusal \emph{induction} on benign prompts).
    \item Base (non-instruction-tuned) model behavior under steering.
    \item Multi-rater reliability measurement for output classification.
    \item Mechanistic validation (SAE analysis, circuit-level probing, or causal interventions at the attention-head level).
    \item Temperature $> 0$ decoding, which real deployments use and which may interact with steering in unknown ways.
    \item Single-layer steering interventions (our protocol applies steering from layer $l$ through the final layer, a technical constraint of our implementation rather than a principled design choice).
\end{itemize}
...
Under our tested protocol, steering effectiveness is at ceiling for smaller models and declines at larger scales. This is consistent with the possibility that the ``single refusal direction'' picture becomes a less complete description at larger scales, though the small sample sizes (n=30 prompts, n=3 architecture families, n=1 failure case) mean this interpretation should be held tentatively. The Mistral failure tells us that steerability is not a universal property of instruction-tuned models but depends on architectural details we do not yet understand. The transfer results sharpen this picture: same-family directions transfer across scales within the tested pair (Qwen 14B $\leftrightarrow$ 32B), but cross-family transfer collapses despite matched hidden dimensionality (Qwen 7B $\leftrightarrow$ Gemma 9B), suggesting that refusal geometry is architecture-specific in the pairs we tested. The tooling sensitivity finding reminds us that the directions we extract are mediated by implementation details that the literature rarely specifies.
