#!/usr/bin/env python3
"""
Phase 2 B2: Cross-family direction transfer (Qwen ↔ Gemma).

Extracts DIM refusal direction from source model family, applies to target family.
Key test: Qwen 7B ↔ Gemma 9B (both hidden_dim=3584, direct behavioral transfer).
"""
...
MODELS: Dict[str, ModelCfg] = {
    ...
    "gemma-9b": ModelCfg("google/gemma-2-9b-it", 16, 3584, 25.0),
}

# Multiplier policy: always use TARGET model's multiplier
...
def eval_steered(model_id: str, direction: np.ndarray, layer_idx: int, multiplier: float) -> dict:
    ...
    for i, p in enumerate(EVAL_PROMPTS):
        handles = [layers[j].register_forward_hook(hook_fn) for j in range(layer_idx, len(layers))]
        try:
            ...
            per_prompt.append({
                "prompt_id": i,
                "prompt": p[:80],  # truncated for artifact size
                "quality": q,
                "refusal": r,
                "output_hash": hex(hash(txt))[-8:],
            })
        finally:
            for h in handles:
                h.remove()
...
def run_transfer(
    ...
    """Run a single cross-family transfer experiment."""
    _set_seed(seed)
    import sys
    sys.path.insert(0, str(ROOT))
    from src.prompts import EVAL_PROMPTS
    ...
    logger.info(f"B2 transfer: {source_key} -> {target_key} (mode={mode})")
    t0 = time.time()
    ...
    # Geometric comparison (always extract target direction for cross-model comparison)
    if not is_self:
        ...
        if dim_match:
            ...
            cos = None  # Can't compare directions with different dims
    ...
    if mode in ("self_control", "direct_transfer"):
        logger.info(f"Running behavioral eval on {target_key} with {'own' if is_self else source_key} direction, mult={effective_multiplier}")
    ...
    if is_self:
        prefix = f"b2_self_{source_key}"
    elif mode == "geometric_only":
        prefix = f"b2_geometric_{source_key}_vs_{target_key}"
    else:
        prefix = f"b2_transfer_{source_key}_to_{target_key}"
...
try:
    ...
    def run_small(source: str, target: str, geometric_only: bool = False, multiplier: float = 0.0, seed: int = 42):
        ...
        path = f"{VOLUME_PATH}/b2_{source}_to_{target}{m_tag}{s_tag}_{stamp}.json"
    ...
    def main(source: str = "qwen-7b", target: str = "gemma-9b", geometric_only: bool = False, multiplier: float = 0.0, seed: int = 42):
        # Pick GPU based on largest model
        sizes = {"qwen-7b": 1, "qwen-14b": 2, "gemma-9b": 1}
        max_size = max(sizes.get(source, 1), sizes.get(target, 1))
        ...
        stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        p = local_dir / f"b2_{source}_to_{target}_{stamp}.json"
        with open(p, "w") as f:
